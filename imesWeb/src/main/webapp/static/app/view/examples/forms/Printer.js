Ext.define('KitchenSink.view.examples.forms.PrinterEdit' , 	{ 
		    extend:  'Ext.Window',
		    alias: 'chmade.PrinterEdit',
		    refreshStore : null ,
			  ...
			  allowBlank:false
    		        ...
    		        buttons: [{
    		         ...
    		         ,   handler: function() {
    		                ...
    		                var beanModel = Ext.create('model.SysPrinterModel',  formValues);
    		                beanModel.save({
...
Ext.define('KitchenSink.view.examples.forms.Printer', {
    extend:  'Ext.panel.Panel',
    alias: 'chmade.sysPrinter',
    header: false,
    ...
    beforeRender: function() {
        var me = this;
        me.callParent();
     	pluginStore =  Ext.create('SysPrinterStore');
				...
				{ 
				  ...
				  name:'resolution'
       		        ...
       		        buttons: ['->', {
                       	...
                       	handler: function() {
		                       		 var form = this.up('form').getForm();
		                       		 var 	 printer = form.getValues();
		                           	var st = 	 this.up('gridpanel').getStore();
		                       		st.on('beforeload', function (store, options) {
		                       			var new_params = printer;
		                       			Ext.apply(store.proxy.extraParams, printer) ;
		                       		}) ;
		                       		st.currentPage=1 ;
		                       		st.load( ) ;
                       	}
       		        ...
       		        },
       		        {
       		            text: 'New',
       		            	handler: function() {
       		            		...
       		            		var   constrainedWin = Ext.create(  'chmade.PrinterEdit', { title:'Add Printer', constrainTo : p.getEl() , refreshStore: this.up('gridpanel').getStore() } );
       		            		constrainedWin.show();
       	                	}
       		        },{
        		            ...
        		            text: 'Edit',
        		            	handler: function() {
        		            		...
        		            		if  ( selModel.length == 1 ) {
            		            		var p =  this.up('gridpanel').up().up() ;
            		            		var   constrainedWin = Ext.create(  'chmade.PrinterEdit', { title:'Edit Printer', constrainTo : p.getEl() , refreshStore: this.up('gridpanel').getStore()  } );
        		            ...
        		            text: 'Delete',
        		            	handler: function() {
        		            		...
        		            		for(var k=0 ;k<selModel.length ;k++){
        		            			...
        		            			selModel[k].destroy({
        		            			    success: function() {
        		            			        console.log('The Printer was destroyed!');
        		            			    }
        		            			});
        		            		}
